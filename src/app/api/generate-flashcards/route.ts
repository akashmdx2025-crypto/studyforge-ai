...
export async function POST(req: NextRequest) {
  ...
  try {
    const { count = 10, context = [] } = await req.json();
    ...
    const chunks = context.length > 0 
      ? [...context].sort(() => Math.random() - 0.5).slice(0, Math.min(12, context.length)).map((e: any) => e.text)
      : vectorStore.getRandomChunks(Math.min(12, vectorStore.size));
