...
export class InMemoryVectorStore {
  ...
  static searchStatic(entries: VectorEntry[], queryEmbedding: number[], topK: number = 3): SearchResult[] {
    return entries
      .map(entry => ({
        entry,
        score: this.cosineSimilarityStatic(queryEmbedding, entry.embedding),
      }))
      .sort((a, b) => b.score - a.score)
      .slice(0, topK);
  }

  private static cosineSimilarityStatic(a: number[], b: number[]): number {
  ...
  search(queryEmbedding: number[], topK: number = 3): SearchResult[] {
    return InMemoryVectorStore.searchStatic(this.entries, queryEmbedding, topK);
  }
